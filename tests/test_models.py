...
def test_model_1_1():
    ...
    assert validate_fit(model["lambda"], H0, PARAMS["1:1"], model["initial_guess"], model["bounds"], rtol=3e-2)

def test_model_1_2():
    model = model_definitions(H0, G0, d_delta_exp)["1:2"]
    assert validate_fit(model["lambda"], H0, PARAMS["1:2"], model["initial_guess"], model["bounds"], rtol=3e-2)

def test_model_2_1():
    model = model_definitions(H0, G0, d_delta_exp)["2:1"]
    assert validate_fit(model["lambda"], H0, PARAMS["2:1"], model["initial_guess"], model["bounds"], rtol=3e-2)

def test_model_dimer():
    model = model_definitions(H0, G0, d_delta_exp)["dimer"]
    assert validate_fit(model["lambda"], H0, PARAMS["dimer"], model["initial_guess"], model["bounds"], rtol=3e-2)

def test_model_multi():
    model = model_definitions(H0, G0, d_delta_exp)["multi"]
    assert validate_fit(model["lambda"], H0, PARAMS["multi"], model["initial_guess"], model["bounds"], rtol=3e-2)
